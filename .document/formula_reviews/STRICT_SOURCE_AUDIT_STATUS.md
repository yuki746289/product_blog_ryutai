...
`fem_7_1_1_reaudit.html` は元画像と再転記MathJaxを左右に並べる比較ページで、原画像上の誤記候補も勝手に訂正せず保持している。
...
1. `fem_7_2_1.html` — 20式のPass 1。
2. `fem_7_1_2.html` — 42式のPass 1。既知の誤記候補を原文と補足へ分離。
...
主要5ページ143式について元画像と監査候補の対応付けは143/143完了。Pass 1合格は32/143。通常ページは元数式画像表示を優先し、MathJax再反映は各ページのPass 1 / Pass 2完了後に行う。
