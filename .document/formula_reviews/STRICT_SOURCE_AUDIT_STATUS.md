...
2. **Column 5画像 / Counting 2画像**を同じ手順で処理する。
3. FEM 33 / Heat 8 / Appendix 9 の残存画像について、既知数式完了後の最終「保持図」確認を行う。
4. **MPS欠損49参照**は数式変換と切り分け、元ファイルの所在・復元可否を調査する。正本なしで式を推測しない。
5. サイト全体の残存画像を最終再スキャンし、未確認数式画像0件を確認する。
6. 全サイトPC/スマホ最終QA後にのみ本番FTP反映を検討する。
...
- **Physicsカテゴリの既知数式画像は変換完了**
  - 最新残存16画像はすべて確認済みの保持図（数式画像0）\n- 厳密監査・通常ページ化まで完了した数式・数式テキスト: **538件**
