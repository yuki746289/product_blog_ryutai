...
| 2 | `fem/fem_7_1_1.html` | 20 | `image002`残存MathJaxを表示時に元画像へガード | `fem/fem_7_1_1_mathjax_audit.html` | **20/20** | **19/20** | **0/20** | 0/20 | **19式NG / image011保留 / 元画像忠実再転記が必要** |
...
|  | **合計** | **143** |  |  | **143/143** | **31/143** | **12/143** | **0/143** | **未完了** |
...
元画像を直接表示して監査候補と比較し、**19/20式を目視監査済み**。19式すべてで不一致を検出した。`image011` は元画像の表示用再取得が未完了のため、推測で判定せず保留している。
...
結果: **Pass 1監査19/20、Pass 1合格0/20、要修正19/20、保留1/20。**

このページは現監査候補の微修正ではなく、20式を元画像忠実転記で再作成する。
...
1. `fem_7_1_1.html` — `image011` を再取得して20/20判定を完了後、20式を元画像忠実転記で再作成。
