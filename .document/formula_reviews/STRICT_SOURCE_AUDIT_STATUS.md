...
1. **全サイト最終Browser QA**を最新 `develop` で完走し、MPSの既知HOLD以外のhard failureを0にする。
2. **MPS欠損49参照**はSOURCE BLOCKEDとして維持する。Libraryの `HP.7z` を実行環境へ展開可能になった場合のみ、Word/元画像を正本として監査を再開する。
3. 最終残存画像再スキャンで、**非MPSの残存112配置がすべて確認済み保持図**、MPS 49のみHOLDであることを固定する。
4. 本番FTP反映は、最終QA結果とMPS HOLD状態を確認した上でのみ検討する。公開後はスモークテストを実施する。
