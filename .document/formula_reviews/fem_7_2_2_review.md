...
- 対象数: 49式
- 状態: **元画像完全一致の再監査対象 / 対応付け49/49確認済み**
...
なお、監査候補には `\sum`、`\mathbf b_S` 等を用いた一般化・補足済み表現が含まれている箇所がある。これらが元画像と一致するかは**対応付け確認だけでは判定せず、Pass 1で必ず原画像と照合する**。
...
**未完了 / 対応付け49/49、Pass 1 = 0/49、Pass 2 = 0/49。**
